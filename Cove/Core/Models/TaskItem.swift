...
struct TaskItem: Identifiable, Hashable, Sendable {
    ...
    /// The app's one task ordering: due date, then time (date-only first
    /// within a day), then note title and line.
    ///
    /// Zero-padded `YYYY-MM-DD` strings order chronologically for free, and
    /// the sentinel puts undated tasks after every dated one — so a list reads
    /// "what's scheduled, then the rest in the order I added it".
    ///
    /// It lives on `TaskItem` rather than on `VaultIndex` because it is a
    /// comparator over two tasks and nothing else, and because the widget
    /// extension sorts with it while compiling none of the index types.
