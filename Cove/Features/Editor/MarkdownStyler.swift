...
enum MarkdownStyler {
    ...
    static func applyLiveStyles(to textStorage: NSTextStorage) {
        applyLiveStyles(to: textStorage,
    ...
    static func applyLiveStyles(to textStorage: NSTextStorage,
        ...
        let range = expandedParagraphRange(in: textStorage.string,
                                           around: dirtyRange,
    ...
    private static func expandedParagraphRange(in text: String,
                                               around dirtyRange: NSRange,
                                               includeNeighbors: Bool) -> NSRange {
        ...
        ns.getParagraphStart(&start, end: &end, contentsEnd: &contentsEnd,
                             for: NSRange(location: safeLocation,
                                          length: safeEnd - safeLocation))
        if includeNeighbors, start > 0 {
            var previousStart = 0
            ns.getParagraphStart(&previousStart, end: nil, contentsEnd: nil,
                                 for: NSRange(location: start - 1, length: 1))
            start = previousStart
        }
        if includeNeighbors, end < ns.length {
            var nextEnd = 0
            ns.getParagraphStart(nil, end: &nextEnd, contentsEnd: nil,
